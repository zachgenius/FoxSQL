...
class LSLProceduresController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView?.addPullToRefreshWithAction({[unowned self] in
            ...
            self.parentController?.reqDelegate?.lslRequestProcedures(self.currentDB!.name)
        })

    }
}

extension LSLProceduresController : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        else if currentDB?.procedures == nil{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if sectionKeys.count > 0 {
            ...
            let title = sectionItemKeys[key]![indexPath.row]
            parentController?.reqDelegate?.lslShowCreate(.procedure, db: currentDB!.name, toShow: title)
        }
    }
...
extension LSLProceduresController : OpeningDBListCellDelegate{
    func onMenuClick(_ anchorView:UIView, _ indexPath: IndexPath) {
        guard let list = currentDB?.procedures else {
            return
        }
        ...
        FTPopOverMenu.showForSender(sender: anchorView,
                with:  [
                    ...
                    switch index {
                    case 0: // view create statement
                        self.parentController?.reqDelegate?.lslShowCreate(.procedure, db: db, toShow: item)
                        break
                    case 1: // drop
                        self.parentController?.reqDelegate?.lslShowDangerProcessAlert("Drop Stored Procedure", hint:"Are you sure to remove the stored procedure?", sql: "DROP PROCEDURE " + item + ";", db: db)
                        break
                    default:
