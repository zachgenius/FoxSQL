...
class LSLFunctionsController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView?.addPullToRefreshWithAction({[unowned self] in
            ...
            self.parentController?.reqDelegate?.lslRequestFunctions(self.currentDB!.name)
        })

    }
}

extension LSLFunctionsController:UITableViewDelegate, UITableViewDataSource{
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        else if currentDB?.functions == nil{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if sectionKeys.count > 0 {
            ...
            parentController?.reqDelegate?.lslShowCreate(.function, db: currentDB!.name, toShow: title)
        }
    }
...
extension LSLFunctionsController:OpeningDBListCellDelegate{
    func onMenuClick(_ anchorView:UIView, _ indexPath: IndexPath) {
        guard let list = currentDB?.functions else {
            return
        }
        ...
        FTPopOverMenu.showForSender(sender: anchorView,
                with: [
                    ...
                    case 0: // view create statement
                        self.parentController?.reqDelegate?.lslShowCreate(.function, db: db, toShow: item)
                        break
                    case 1: // drop
                        self.parentController?.reqDelegate?.lslShowDangerProcessAlert("Drop Function", hint:"Are you sure to remove the function?", sql: "DROP FUNCTION " + item + ";", db: db)
                        break
                    default:
